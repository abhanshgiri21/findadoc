...
router.get('/', function(req, res, next) {
  if(req.query.state){
      ...
      client.execute(query, [req.query.state] , function(err, results){
        ...
        res.render('doctors', {title:'doctors', doctors:results.rows});
    });
  } else{
  ...
  client.execute(query, [], function(err, results){
    if(err){
      res.status(404).send({msg:err});
        ...
        res.render('adddoctor', {title:'adddoc', categories:results.rows});
    }
  });
});
...
router.get('/details/:id', function(req, res, next) {
  ...
  client.execute(query, [req.params.id], function(err, result){
      ...
      res.render('details', {title:'doctors', doctor:result.rows['0']});
  })
